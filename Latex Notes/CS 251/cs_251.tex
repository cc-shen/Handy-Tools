...
\begin{document}
  ...
  To an individual, the performance in interest is \defnterm{response time}---the time between the start and completion of a task---also referred to as \defnterm{execution time}. \\
  To a data-center manager, the interest is in \defnterm{throughput} or \defnterm{bandwidth}---the total amount of work done in a given time.
  ...
  Program execution time is measured in seconds per program, this is usually known as \defnterm{elapsed time}, wall clock time, or \defnterm{response time}. \\
  ...
  The \defnterm{CPU execution time}, also knowns as \defnterm{CPU time}, makes the distinction between the elapsed time and the actual time the CPU spends computing for a specific task. \\
  Further distinction can be made, \defnterm{user CPU time}, the CPU time spent in the program, and \defnterm{system CPU time}, the CPU time spent in the operating system performing tasks on behalf of the program. \\
  ...
  These discrete time intervals are called \defnterm{clock cycles} (or ticks, clock ticks, clock periods, clocks, cycles). \\
  ...
  \defnterm{Clock cycles per instruction}, which is the average number of clock cycles each instruction takes to execute, is often abbreviated as \textbf{CPI}. \\
  ...
  A group of eight bits is called a \defnterm{byte}. Represents $256 = 2^{8}$ possibilities. \\
  A group of four bits is called a \defnterm{nibble}. Represents $16 = 2^{4}$ possibilities. \\
  ...
  \defnterm{Noise margin} is the amount of noise that could be added to a worst-case output such that the signal can still be interpreted as a valid input.
  ...
  Two main types of transistors are \defnterm{bipolar transistors} and \defnterm{metal-oxide-semiconductor field effect transistors} (MOSFETs or MOS transistors).
  ...
  The n-type transistors, called \defnterm{nMOS}, have regions of n-type dopants adjacent to the gate called the \defnterm{source} and the \defnterm{drain} and are built on a p-type semiconductor substrate. \\
  The \defnterm{pMOS} transistors are just the opposite, consisting of p-type source and drain regions in an n-type substrate. \\
  ...
  These processes that provide both flavors of transistors are called \defnterm{Complementary MOS} or CMOS.
...
binary and those who can't.} \\
  ...
  The \defnterm{complement} of a variable, $A$, is its inverse, $\overline{A}$. \\
  The variables or its complement is called a \defnterm{literal}.
  For example, $A$, $\overline{A}$, $B$, and $\overline{B}$ are literals. \\
  $A$ is called the \defnterm{true form} of the variable and $\overline{A}$ the complementary form; ``true form'' does not mean that $A$ is TRUE, but merely that $A$ does not have a line over it. \\

  The AND of one or more literals is called a \defnterm{product} or an \defnterm{implicant}. \\
  $\overline{A}B$, $A\overline{B} \overline{C}$, and $B$ are all implicants for a function of three variables. \\
  A \defnterm{minterm} is a product involving all of the inputs to the function.
  ...
  The OR of one or more literals is called a \defnterm{sum}. \\
  A \defnterm{maxterm} is a sum involving all of the inputs to the function.
  ...
  This is called the \defnterm{sum-of-products canonical form} of a function because it is the sum (OR) of products (ANDs forming minterms). \\
  ...
  However, other bistable elements, such as \textbf{latches} and \textbf{flip-flops}, provide inputs to control the value of the state variable.
  ...
  \defnterm{SR latch} is one of the simplest sequential circuit as it is composed of two cross-coupled NOR gates. \\
  ...
  A D flip-flop is also known as a \defnterm{master-slave flip-flop}, an \defnterm{edge-triggered flip-flop}, or a \defnterm{positive edge-triggered flip-flop}.

  \subsubsection{Register}
  A \emph{N}-bit register is a bank of \emph{N} flip-flops that share a common \emph{CLK} input, so that all bits of the register are updated at the same time. \\
  ...
  A \defnterm{register file} is a way of organizing registers. \\
  ...
  Synchronous sequential circuits can be drawn in the forms called \defnterm{finite state machines} (FSMs).
  ...
  In \defnterm{Moore machines}, the outputs depend only on the current state of the machine. \\
  In \defnterm{Mealy machines}, the outputs depend on both the current state and the current inputs.
  ...
  A \defnterm{state transition diagram} indicates all the possible states of the system and the transitions between these states. \\
  ...
  A \defnterm{state transition table} indicates for each state and input, what the next state, $S'$, should be.
  If the next state does not depend on a particular input, the don't care symbol (X) is used. \\
  To build a real circuit, the states and outputs must be assigned a \defnterm{binary encoding}.
  ...
  With \defnterm{binary encoding}, each state is represented as a binary number.
  Because $K$ binary numbers can be represented by $\log_{2}K$ bits, a system with $K$ states only needs $\log_{2}K$ bits of state. \\

  In \defnterm{one-hot encoding},  a separate bit of state is used for each state.
  ...
  Another encoding is the \defnterm{one-cold} encoding, in which $K$ states are represented with $K$ bits, exactly one of which is FALSE.
  ...
  \textbf{exponent} is the value of the 8-bit exponent field (including the sign of the exponent).
  \textbf{fraction} is the 23-bit number. \\
  ...
  \blfootnote{\defnterm{overflow}: positive exponent becomes too large to fit in the exponent field}
  \blfootnote{\defnterm{underflow}: negative exponent becomes too large to fit in the exponent field}

  A \textbf{double precision} floating-point number is represented in two 32-bit words.
  ...
  This is also known as the \textbf{IEEE 754 floating-point standard}.
