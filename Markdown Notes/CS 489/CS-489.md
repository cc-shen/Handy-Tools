...
_Assume there exists some ${\bf w}$ such that $A^{\intercal}{\bf w} > 0$_, then the perception algorithm converges to some $w^{*}$. If each column of $A$ is selected indefinitely often, then $A^{\intercal}{\bf w}^{*} > \delta$.
...
**Thm.** (Littlestone'88)  
Assume there exists some _nonnegative_ ${\bf w}$ such that $A^{\intercal}{\bf w} > 0$, then winnow converges to some ${\bf w}^{*}$. If each column of $A$ is selected indefinitely often, then $A^{\intercal}{\bf w}^{*} > \delta$

## Lecture 02: , _September 14, 2017_
