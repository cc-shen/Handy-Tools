...
In this variation, we are given a weighted directed graph, represented as a matrix. \\
Our goal is to find the shortest path (i.e., the path of smallest total weight) from vertex $i$ to vertex $j$, \emph{for all pairs i, j}.
(Before we wanted the shortest paths from a \emph{single vertex} to all other vertices.) \\

\subsubsection{Path Algebra}
With "path algebra" we can solve the following three problems on directed graphs, and many more:
\begin{itemize}
  \item Compute the total number of paths between all pairs of vertices in a directed acyclic graph
  \item Solve the "all-pairs-shortest-paths" problem in a weighted directed graph with no negative cycles
  \item Compute the "transitive" closure of a directed graph
\end{itemize}

The principal idea in path algebras is that matrix multiplication (and things like matrix multiplication) of adjacency matrices of directed graphs give important information about the graphs. \\
